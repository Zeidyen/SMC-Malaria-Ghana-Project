...
# OPTIONS ###############################################################
#########################################################################
site_name = 'Northern_Region'                                           # Existing or new site to request climate data for
birth_rate= 33.0                                                        # Birth Rate (per 1,000) to apply for FIXED_BIRTH_RATE, which is default in the simulation below
...
sim_start_year=2023
memory_limit = 10000                                                    # in MB, limit set for EMOD config parameter and QUEST resource allocation 
##### Experiment Specifications #########################################
exp_label = 'Northern_prediction'                                                          # added label for outputs
burnin_id = '7635fb9a-813d-4ce8-a474-6ecf35942626'                                                        # Single-Node Ex. hab_sweep_1 (site 'sapone' with climate from 2010-2019)
pickup_id = ''                                                          # 
...
##################################################### Interventions #####
cm_file = 'interventions_CM_p.csv'                                        # Case management: coverage by age
smc_file = 'interventions_SMC_p additional_cycle_late.csv'                                      # SMC: coverage by round
itn_file = 'interventions_ITN_p.csv'                                      # Bednets: distribution coverages and insecticide properties
itn_age_file = 'interventions_ITN_age_p.csv'                              # Bednets: Age-specific usage
itn_season_file = 'interventions_ITN_season_p.csv'                        # Bednets: Seasonal usage
...
scale_water_veg = [0.1,2.5,1.0,0.0]                                 # Amount of Water_Vegetation relative to Constant 
scale_temp_rain = [25,100,50]                            # Amount of Temporary_Rainfall relative to Constant 
scale_constant = [1.0]                                                  # Amount of Constant relative to Base of 5e5. Typically left as [1.0] 
xTLH_range = list(np.logspace(1.85,2.15,5))	                       # Note: x_Temporary_Larval_Habitat scales ALL habitat types, including constant
#xTLH_range=list(np.linspace(130, 160, 20))
#list(np.logspace(1.85,2.15,20)) # 
            ...
            if len(smc_df) > 0:                                                                                            #
                for r in range(len(smc_df)):                                                                               # For each row (round) in smc_file...
                    ...
                    dc.add_drug_campaign(camp, campaign_type="MDA", drug_code="SPA", node_ids =[int(node)],                     # ... Add SMC with drug SPA
                                         ...
                                         repetitions=1,
                                         coverage=float(smc_df['coverage'][r]),                                            # ... at this coverage
                                         target_group={'agemin': 10, 'agemax': 15},
                                         receiving_drugs_event_name="Received_SMC") 
...
def general_sim(selected_platform):                                                                                        # general_sim() - connecting all of the simulation steps
    ...
    if(step=="burnin"):                                                                                                    # Specifications for burnin simulations 
        platform = Platform(selected_platform, job_directory=manifest.job_directory,                                       # 
                            ...
                            time='6:00:00',max_running_jobs=100, mem=memory_limit,                                         # (do not exceed 100 max_running_jobs)
                            modules=['singularity'])                                                                       #
    if(step=="pickup"):                                                                                                    # Specifications for pickup simulations
        platform = Platform(selected_platform, job_directory=manifest.job_directory,                                       #                                            
                            ...
                            time='4:00:00',max_running_jobs=100, mem=memory_limit,                                         #
                            modules=['singularity'])                                                                       #
    ##### Task #############################################################################################################
    ...
    if(step == 'pickup'):                                                       #Pickup Reports:
        ...
        demo = pd.read_csv(os.path.join(manifest.input_dir,site_name,'demographics.csv'))                                                   #    - Maximum Age
        if os.path.exists(os.path.join(manifest.input_dir,site_name,"migration")):                                         #

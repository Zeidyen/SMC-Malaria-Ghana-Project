...
exp_label = 'Northern_region_TemporaryRainfall'                                                          # added label for outputs
burnin_id = '900297c1-3f90-4bc6-b6d1-15e56acb7048'                                                        # Single-Node Ex. hab_sweep_1 (site 'sapone' with climate from 2010-2019)
pickup_id = ''                                                          # 
...
habitat_df = pd.DataFrame(scale_temp_rain, columns=['scale_temp_rain'])
n_habs = len(scale_temp_rain)                                     #
#########################################################################
#########################################################################
#########################################################################
...
#########################################################################
################################################### #####################
def scale_habitat_from_df(df=habitat_df, index=-1, vdf=None):
    scale_temp_rain = 1  # Default value
    if(index >= 0):
        scale_temp_rain = df['scale_temp_rain'][index]  # Retrieves the scale_temp_rain value from the DataFrame
    ...
    if vdf is not None:
        ...
        for species in s:
            ...
            # Assuming TEMPORARY_RAINFALL habitat type needs to be scaled, adjust as needed
            all_scales[f"TEMPORARY_RAINFALL.{species}"] = [scale_temp_rain * f for f in sub_vdf['fraction']]
...
def update_campaign_multiple_parameters(simulation, habitat_index, scale_start_day):                                       # update_campaign_multiple_parameters()
    ...
    return {"scale_temp_rain": float(habitat_df['scale_temp_rain'][habitat_index])}
                                                                                                                           # #################################
def update_serialize_parameters(simulation, df, x: int):                                                                   # update_serialization_parameters()
    ...
    # scale_constant = float(df["scale_constant"][x])                                                                      # ...scale_constant from burnin (Not needed)
    scale_temp_rain = float(df["scale_temp_rain"][x])                                                                      # ...scale_temp_rain from burnin
    # scale_water_veg = float(df['scale_water_veg'][x])                                                                    # ...scale_water_veg from burnin (Not needed)
    # Assuming habitat_df has been updated to only include scale_temp_rain
    h_index = habitat_df.index[habitat_df["scale_temp_rain"] == scale_temp_rain][0]                                        # -> Find matching habitat_index -> 

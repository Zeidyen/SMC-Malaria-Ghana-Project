...
step = 'analyze_burnin'                                                         # Which step of the process: setup_climate? setup_migration? 
...
exp_label = 'NorthernRegion_Pickup'                                               # added label for outputs
burnin_id = '8d2c40fe-05f0-407f-951f-dd1e73a2a729'                      # Single-Node Ex. hab_sweep_1 (site 'sapone' with climate from 2010-2019)
pickup_id = '8d2c40fe-05f0-407f-951f-dd1e73a2a729'                      # 
...
def general_sim(selected_platform):                                                                                        # general_sim() - connecting all of the simulation steps
    ...
    if(step == 'pickup'):                                                                                       #Pickup Reports:
        ...
        end_report = pickup_years*365+30                                                                                   # **
        ...
        for n in demo['node_id'].unique(): 
            add_report_event_counter(task, manifest,
                                     node_ids = [int(n)],  filename_suffix="_".join(('node',str(n))),                             
                                     start_day = start_report,                                                             #
                                     end_day = end_report,                                                                 #
...
def analyze_pickup(pickup_id=pickup_id, output_folder=output_folder,                               #
                   ...
                   EventCounter = True,
                   InsetChart = True, channels_inset_chart=channels_inset_chart, start_year = 1950,# 
...
if __name__ == "__main__":                                                                          #
    ...
    if step =='analyze_burnin':                                                                     # Analyze burnin (only during testing, otherwise no output generated)
        ...
        analyze_pickup(pickup_id=burnin_id, output_folder=output_folder,                            #
                       sweep_variables = sweep_variables,                                           #
                       InsetChart = IC,channels_inset_chart=channels_inset_chart,start_year = 1950, #
                       MonthlyReport = True, report_years=report_years)                             #
    if step in ('calibrate','calibration'):                                                         # Analyze pickup and score against calibration targets
        analyze_pickup(pickup_id=pickup_id, output_folder=output_folder,                            #
                       sweep_variables = sweep_variables,                                           #
                       InsetChart=True,channels_inset_chart=channels_inset_chart,start_year = 2000, # 
                       MonthlyReport=True, report_years=report_years)                               #
